...
(def ^:const output-file "../related_posts_techascent.json")
...
(defn count-tags!
  [^java.util.Map post ^ints tagged-post-count ^HashMap tag-map]
  (let [^List tags (.get post "tags")
        ntags (.size tags)]
    (dotimes [i ntags]
        ...
        (dotimes [j sz]
          (let [idx (aget arr j)]
            (aset tagged-post-count idx
                  (unchecked-inc-int (aget tagged-post-count idx)))))))))

(defn get-all-related-posts [^objects posts]
  (let [n (alength posts)
        ^HashMap tag-map (make-tag-map posts)
        ^ints tagged-post-count (int-array n)
        ^objects results (object-array n)]
    (dotimes [post-idx n]
      (let [^java.util.Map post (aget posts post-idx)]
        (java.util.Arrays/fill tagged-post-count (int 0))
        (count-tags! post tagged-post-count tag-map)
        (aset tagged-post-count post-idx (int 0))
        (let [^longs top5 (get-top-5 tagged-post-count n)
